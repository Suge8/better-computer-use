...
Managed browser launch currently resolves macOS application paths only; on Linux, attach to an existing browser instead. Manual CDP launches must use a non-default `--user-data-dir` together with `--remote-debugging-port`; set `BCU_CDP_PORT` to that loopback port.
