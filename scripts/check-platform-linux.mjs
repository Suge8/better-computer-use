...
const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bcu-linux-platform-"));
...
process.env.BCU_LINUX_HELPER_PATH = fakeHelper;
...
const readline = require("node:readline");
fs.appendFileSync(process.env.BCU_TEST_LINUX_START_COUNT, "start\\n");
readline.createInterface({ input: process.stdin }).on("line", (line) => {
...
const fs = require("node:fs");
fs.appendFileSync(process.env.BCU_TEST_LINUX_INSTALL_COUNT, "install\\n");
setTimeout(() => fs.chmodSync(process.env.BCU_LINUX_HELPER_PATH, 0o755), 100);
`);
	process.env.BCU_TEST_LINUX_INSTALL_COUNT = installCount;
	process.env.BCU_TEST_LINUX_START_COUNT = startCount;
	...
	delete process.env.BCU_TEST_LINUX_INSTALL_COUNT;
	delete process.env.BCU_TEST_LINUX_START_COUNT;
