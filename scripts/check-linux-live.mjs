...
if (process.env.BCU_LIVE !== "1") {
	throw new Error("Live Linux E2E is disabled. Set BCU_LIVE=1 to run it explicitly.");
}
...
if (!process.argv.includes("--inside-session")) {
	const child = spawn("dbus-run-session", ["--", process.execPath, "--import", "tsx", scriptPath, "--inside-session"], {
		env: { ...process.env, BCU_HEADLESS: "1", BCU_BROWSER_USE: "0" },
		stdio: "inherit",
	});
...
async function runInsideSession() {
	...
	const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bcu-linux-live-"));
	const processes = [];
...
function start(command, args, env) {
	...
	child.stderr.on("data", (chunk) => {
		if (process.env.BCU_LIVE_DEBUG === "1") process.stderr.write(chunk);
	});
	return child;
}
...
function debug(label, value) {
	if (process.env.BCU_LIVE_DEBUG === "1") console.error(`[linux-live] ${label}: ${JSON.stringify(value)}`);
}
