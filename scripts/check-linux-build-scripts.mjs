...
assert.match(setupSource, /BCU_LINUX_HELPER_PATH/);
assert.match(setupSource, /prebuilt", "linux", arch, "linux-bridge"/);
assert.match(setupSource, /allowLinuxBuildFallback = args\.has\("--allow-build"\) \|\| process\.env\.BCU_ALLOW_BUILD === "1"/);
assert.match(helperSource, /\.pi", "agent", "helpers", "bcu", "linux-bridge"/);
...
const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "bcu-linux-scripts-"));
...
try {
	...
	const result = await new Promise((resolve) => {
		const child = spawn(process.execPath, [fixtureEntry, "--platform", "linux"], { env: { ...process.env, BCU_LINUX_HELPER_PATH: helperDest }, stdio: ["ignore", "pipe", "pipe"] });
		let output = "";
